...
Printing::CPntGridCtrl::CPntGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols, CDC* pDC)
{
	m_pCellDefault = new CPntGridDefaultCell(pDC);
...
Printing::CPntGridCtrl::~CPntGridCtrl()
{
	...
	if (m_pCellDefault != NULL)
	{
		delete m_pCellDefault;
		m_pCellDefault = NULL;
	}
}
...
BOOL Printing::CPntGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle)
{
	...
	int i;
	for (i = 0; i < m_nRows; i++)
		m_arRowHeights[i] = m_pCellDefault->GetHeight();
	for (i = 0; i < m_nCols; i++)
		m_arColWidths[i] = m_pCellDefault->GetWidth();
...
{
	m_pCellDefault->SetGrid(this);            // Normal editable cell
	...
	m_pCellDefault->SetTextClr(m_crWindowText);   
	m_pCellDefault->SetBackClr(m_crWindowColour); 
...
LRESULT Printing::CPntGridCtrl::OnGetFont(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	...
	return (LRESULT) m_pCellDefault->GetFontObject()->GetSafeHandle();
}
	...
	TRY
	{
		...
		if (GetVirtualMode())
		{
			...
			if (addedRows > 0)
			{
				...
				for (int row = startRow; row < nRows; row++)
					m_arRowHeights[row] = m_pCellDefault->GetHeight();
			}
			ResetVirtualOrder();
		}
		else
		{
			...
			if (addedRows > 0)
			{
				...
				for (int row = startRow; row < nRows; row++)
				{
					int rowHeight = m_pCellDefault->GetHeight();
...
void Printing::CPntGridCtrl::AutoFill()
{
	...
	SetColumnCount(rect.Width() / m_pCellDefault->GetWidth() + 1);
	SetRowCount(rect.Height() / m_pCellDefault->GetHeight() + 1);
	SetFixedRowCount(1);
...
void Printing::CPntGridCtrl::OnPrint( CDC *pDC, int currentPageNum, CRect clientRect , PntPrintEndResult* printedEndReslt)
{
	...
	while (m_nCurrPrintRow < GetRowCount())
	{
		...
		for (int col = m_nPrintColumn; col < GetColumnCount(); col++)
		{
			...
			if( rect.right > leftMargin + m_nPageWidth)
				break;
	...
	m_bDrawingMergedCell = TRUE;
	beginPrintRow = 0; // for all the cells can be merged, so we should calculate from 0
	INT_PTR size = m_arMergedCells.GetSize();
	if(size > 0)
	{	
		CRect rcMergeRect;
		for(INT_PTR i = 0; i < size; i++)
		{
			...
			if (minRow == 0 && m_bNeedDrawHeading == false)
			{
				if (maxRow > 0)
				{
					minRow++;
				...
				{
					// maxRow == 0 and minRow ==0 means this row should be avoid
					continue;
				}
			}
			...
			{
				// rcMergeRect is based on the (0,0), so it need some adjusion
				int headingHeight = 0;
				if (m_bNeedDrawHeading == false)
				{
					headingHeight = GetRowHeight(0);
				}

				rcMergeRect.left += clientRect.left;
				rcMergeRect.right += clientRect.left;
				rcMergeRect.top = rcMergeRect.top + clientRect.top - headingHeight;
				rcMergeRect.bottom = rcMergeRect.bottom + clientRect.top - headingHeight;
					...
					&& minRow >= beginPrintRow && minRow < m_nCurrPrintRow)
				{
					if (pCell)
					{
						pCell->PrintCell(pDC, minRow, minCol, rcMergeRect);
				//		pCell->Draw(pDC, minRow, minCol, rcMergeRect, TRUE);
					...
					if (m_nGridLines == GVL_BOTH || m_nGridLines == GVL_VERT)
					{
						...
						if (m_arMergedCells[i].GetMinRow() == 0 
							|| (m_bNeedDrawHeading == false && m_arMergedCells[i].GetMinRow() == 1) )
						{
							Overlap = 0;
						}
